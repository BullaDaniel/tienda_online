        ...
        DB.query('SELECT ruta_imagen FROM imagenes_producto WHERE producto_id = ?', [productoId], (errImg, imagenes) => {
            ...
            DB.query('SELECT id, color, talla, stock FROM variantes WHERE producto_id = ?', [productoId], (errVar, variantes) => {
                ...
                DB.query(queryRelacionados, [productoId], (errRel, relacionados) => {
                    ...
                    res.json({ ...producto, imagenes: imagenes.map(i => i.ruta_imagen), variantes, relacionados });
                });
            });
        });
    });
    ...
    DB.query(
        ...
        (err, result) => {
            ...
            DB.query(
                ...
                (errImg) => {
                    if (errImg) return res.status(500).json({ error: errImg.message });
                    const listaTallas = (tallas || '').split(',').map(t => t.trim()).filter(Boolean);
...
app.post('/api/productos/:id/resenas', (req, res) => {
    ...
    let usuario;
    try { usuario = jwt.verify(token, SECRET_KEY); }
...
app.post('/api/colecciones/:id/cards', (req, res) => {
    ...
    DB.query(
        `INSERT INTO coleccion_cards
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [req.params.id, titulo, descripcion_corta || '', imagen || '', enlace || '', precio || null, fecha || null, tags || '', etiqueta || ''],
        (err, result) => {
            if (err) return res.status(500).json({ error: err.message });
            res.status(201).json({
                id: result.insertId,
...
app.put('/api/colecciones/:id/cards/:cardId', (req, res) => {
    ...
    DB.query(
        ...
        [titulo, descripcion_corta || '', imagen || '', enlace || '', precio || null, fecha || null, tags || '', etiqueta || '', req.params.cardId, req.params.id],
        (err) => {
            if (err) return res.status(500).json({ error: err.message });
